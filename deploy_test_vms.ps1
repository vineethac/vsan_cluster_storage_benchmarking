...
Process {
    ...
    for ($i=1; $i -le $hosts_in_cluster.Count; $i++) {
        ...
        $datastore_name = $config_data.datastore_name
        ...
        for ($j=1; $j -le $VM_count; $j++) {
            ...
            New-VM -Name $VM_name -VMHost $host_name -ResourcePool $cluster_name -Datastore $datastore_name -Template $vm_template -Verbose | New-HardDisk -CapacityGB $config_data.disk_size_in_GB -StorageFormat EagerZeroedThick -Persistence persistent -Verbose | New-ScsiController -Type ParaVirtual -Verbose
